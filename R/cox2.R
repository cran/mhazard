...
#' at two specificied values of the covariates. If desired, (bootstrap)
...
#' (meaing that T1out and T2out must both have length 3 if
#' confidencd="CB"). No confidence measures will be returned if
#' confidence="none".
#' @return A list containing the following elements:
#' \describe{
#' \item{n10, n01:}{Total number of events for the first/second outcome}
...
#' x <- genClaytonReg(1000, 2, 0.5, 1, 1, log(2), log(2), log(8/3), 2, 2)
#' x.cox2 <- cox2(x$Y1, x$Y2, x$Delta1, x$Delta2, x$X)
#' x.LF <- cox2.LF(x.cox2, 0, 1, c(0.25, 0.5, 1), c(0.25, 0.5, 1))
...
cox2.LF <- function(cox2.obj, X0_out, X1_out, T1_out, T2_out,
    ...
    Y1 <- cox2.obj$Y1
    Y2 <- cox2.obj$Y2
    Delta1 <- cox2.obj$Delta1
    Delta2 <- cox2.obj$Delta2
    X <- as.matrix(cox2.obj$X)
    n.sub <- length(Y1)
